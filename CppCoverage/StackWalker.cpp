...
{
	this->m_options = OptionsAll & ~SymBuildPath;
...
void StackWalker::Init(const char* strBaseLogDir)
{
	if (strBaseLogDir != nullptr)
	{
		m_strBaseLogDir = strBaseLogDir;
	}
	m_strBaseLogDir.append("/ServerLog/StackInfo");
}
...
BOOL StackWalker::ShowCallstack(HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData, bool bThreadPause)
{
	OnOutput("***************** Start show call stack *****************");
...
{
	printf("%s\n", buffer);
	StackLogSave("ServerLog/StackInfo", buffer);
}

void StackWalker::StackLogSave(const char *pszName, const char *fmt, ...)
{
	if (!pszName || !fmt) {
		return;
	}
